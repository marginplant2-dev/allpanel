...
export function classifyOptions(options: CasinoOption[]): Classified {
  ...
  return {
    main,
    sides: [...sides.entries()]
      .filter(([, v]) => v.chips.length || v.cards.length)
      .map(([name, v]) => ({ name, ...v })),
  };
}
...
function cellState(option: CasinoOption, bettingOpen: boolean) {
  ...
  return {
    open,
    className: open
      ? "bg-ex-back hover:brightness-105 cursor-pointer"
...
export function MainBets({
  ...
  return (
    <div className="grid gap-1 p-1 sm:grid-cols-2 lg:grid-cols-4">
      {options.map((o) => {
        ...
        return (
          <button
            ...
            onClick={() => onSelect(o)}
            className={cn(
              "relative flex min-h-[64px] flex-col items-center justify-center gap-0.5 border border-ex-line px-2 py-2",
              state.className,
              selectedSid === String(o.sid) && "ring-2 ring-inset ring-ex-brand",
            )}
          >
            <span className="text-[13px] font-bold uppercase text-slate-900">{o.name}</span>
...
export function SideBlock({
  ...
  return (
    <section className="border border-ex-line">
      <h3 className="bg-slate-700 px-3 py-1.5 text-center text-[13px] font-bold uppercase text-white">
        {side.name}
      </h3>
